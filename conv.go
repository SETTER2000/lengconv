...
func FtToM(f Foot) Meter {
	return Meter(Meter(f) / 3.281) // преобразование типа Foot в Meter
}

func MToFt(m Meter) Foot {
	return Foot(Foot(m) * 3.281) // преобразование типа Meter в Foot
}
func MToCm(m Meter) Centimeter {
	return Centimeter(Centimeter(m) * 100) // преобразование типа Foot в Meter
}
